...
##########################################################################################
# Compute the 4 solutions
dt = 1 // 20
sol1 = solve(prob, EK1(order=1, diffusionmodel=:fixed), adaptive=false, dt=2dt // 3);
sol2 = solve(prob, EK1(order=1, diffusionmodel=:fixed, manifold=g1), adaptive=false, dt=dt);
sol3 = solve(prob2, EK1(order=2, diffusionmodel=:fixed), adaptive=false, dt=2dt // 3);
sol4 = solve(prob2, EK1(order=2, diffusionmodel=:fixed, manifold=g2), adaptive=false, dt=dt);

##########################################################################################
# Plotting with Makie.jl
fig = Figure(resolution=(250, 220), figure_padding=5)
kwargs = (
    xgridvisible=false,
    ygridvisible=false,
...
for (i, sol) in enumerate((sol1, sol2))
    ...
    ds1 = sol(times).u.μ
    lines!(fig[i, 1], [u[1] for u in ds1], [u[2] for u in ds1], color=(COLORS[i], 0.8))
    scatter!(
        fig[i, 1],
        [u[1] for u in sol.u],
        [u[2] for u in sol.u],
...
for (i, sol) in enumerate((sol3, sol4))
    ...
    ds2 = sol(times).u.μ
    lines!(fig[i, 2], [u[3] for u in ds2], [u[4] for u in ds2], color=(COLORS[i+2], 0.8))
    scatter!(
        fig[i, 2],
        [u[3] for u in sol.u],
        [u[4] for u in sol.u],
